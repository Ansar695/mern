...
const Supplier = () => {
    ...
    const deleteCustomer = async(e) => {
        ...
        if(conf){
            try {
                ...
                const res = await fetch("/delete-supplier", {
                    method: "POST",
                    headers: {
    ...
    const submitInvoice = async(e) => {
        if(invoices.invNo===""||invoices.amount===""||invoices.pay===""||invoices.date===""||invoices.paymentMethod==""){
            alert("Please Enter details")
        }else{
            if(invoices.amount < invoices.pay){
                alert("Please Enter valid amount details")
            }else{
                try {
                    const res = await fetch("/add-supplier-invoice", {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json"
                        },
    ...
    const submitInfo = async(e) => {
        ...
        try {
            const res = await fetch("/add-supplier", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
      ...
      <div className={`menu_bar ${menuBar?'menu_bar_active':''}`}>
        ...
        <div className="right">
          ...
          <div className="bottom">
              <div className="top">
                  <div className="search">
                      <h1>Suppliers List <i className='fa fa-plus'
        ...
        <div className={`overlay ${showOverlay? 'overlay_active': ''}`}>
          <div className="inner">
              ...
              <div className="input">
                  ...
                  <input type="text" name="invNo" value={invoices.invNo} onChange={handleChange}/>
              </div>
              <div className="input">
                  <label>Amount : </label>
